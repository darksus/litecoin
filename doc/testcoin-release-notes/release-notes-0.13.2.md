Testcoin Core version 0.13.2 is now available from:
  ...
  <https://download.testcoin.org/testcoin-0.13.2.1/>
...
Testcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Testcoin Core 0.13.2 also introduces new default policy limits on the length and
...
- `testcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `testcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Testcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Testcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Testcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of testcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of testcoins being spent is exactly
...
- **Increased security for multisig:** Testcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Testcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Testcoin's current ledger (called the UTXO set),
  ...
  note that Testcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Testcoin users to individually opt-in to almost any change in the Testcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Testcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Testcoin
client to Testcoin Core 0.13.2 to signal for segwit support. When a
...
Since Testcoin Core 0.10.0, nodes have defaulted to only relaying and
